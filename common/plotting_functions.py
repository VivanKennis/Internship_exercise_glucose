...
# function to find the rows and columns that are the closest to a square
...
def create_and_load_axis_object(
    ...
    fig_width: float = FIG_WIDTH_A4, fig_height: float = FIG_HEIGHT_A4) -> Axes:
    used_fig_width = float(plot_info.get("fig_width", fig_width))
    used_fig_height = float(plot_info.get("fig_height", fig_height))
    ...
    if fig_num not in figs:
        figs[fig_num] = {
            "fig": plt.figure(
                ...
                figsize=(used_fig_width*(1/2.54), used_fig_height*(1/2.54))
            ),
            "axs": {},
            "legend_position": DEFAULT_LEGEND_POSITION,
            "show_legend": False,
            "legend_subplot_idx": None,
        }

    # a figure only gets a legend when a subplot explicitly requests it.
    if bool(plot_info.get("legend", False)):
        figs[fig_num]["show_legend"] = True

        if "legend_subplot_idx" in plot_info:
            figs[fig_num]["legend_subplot_idx"] = int(plot_info["legend_subplot_idx"])
        elif figs[fig_num].get("legend_subplot_idx") is None:
            figs[fig_num]["legend_subplot_idx"] = int(plot_info["subplot_idx"])

        if "legend_position" in plot_info:
            figs[fig_num]["legend_position"] = plot_info["legend_position"]
...
def add_intensity_lines(ax: Axes, input_values: dict) -> None:
    ...
    # put the last label highest and the first label lowest, so the stack is reversed.
    # this keeps the labels separated vertically without the overlap seen before.
    for i in range(1, len(times)):
        ...
        ax.vlines(tval, y_min, y_max, colors='grey', linestyles='--', linewidth=0.9, alpha=0.9)

        label = f"{intensity_val:g}%"
        label_x = min(tval + dx, x_max)
        step_index = len(times) - i - 1
        y_text = y_max - (0.04 + 0.07 * step_index) * plot_height
        ax.text(
            label_x, y_text, label,
            ha='left', va='top', fontsize=FONT_SIZE_INTENSITY_LABEL,
...
def plot_figures(
    ...
    model,
    fig_names: dict,
    simulate_steady_state: bool = False,
    fig_width_cm: float = FIG_WIDTH_A4,
    fig_height_cm: float = FIG_HEIGHT_A4,
) -> None:
        ...
        for k_obs, obs in d["Observables"].items():
            ...
            if fig_num in fig_names:
                ...
                ax = create_and_load_axis_object(
                    figs, fig_num, plot_info,
                    fig_width=plot_info.get("fig_width", FIG_WIDTH_A4),
                    fig_height=plot_info.get("fig_height", FIG_HEIGHT_A4),
                )
                ...
                ax.plot(sim.time_vector, y_sim, color=c_plot, label="Model", lw=LINE_WIDTH)

                # SEM as error bars instead of a shaded band
                try:
                    mean = np.array(obs['Mean'])
                    sem = np.array(obs['SEM'])
                    ax.errorbar(
                        obs['Time'], mean, yerr=sem, fmt='o', linestyle='none',
                        color=(0.122, 0.467, 0.706), markerfacecolor=(0.122, 0.467, 0.706),
                        markeredgecolor=(0.122, 0.467, 0.706), markersize=5,
                        ecolor=(0.122, 0.467, 0.706), elinewidth=1.5, capsize=3,
                        zorder=5, label="Data")
                    ax.errorbar(
                        obs['Time'], mean, yerr=sem, fmt='none', linestyle='none',
                        color=(0.122, 0.467, 0.706), ecolor=(0.122, 0.467, 0.706),
                        elinewidth=1.5, capsize=3, zorder=4, label="SEM")
                except Exception:
                    pass
    ...
    for fig_num, figure in figs.items():
        ...
        # create a single shared legend for the whole figure to avoid repeating the same legend in every subplot
        handles = []
        labels = []
        for ax in figure["axs"].values():
        ...
        fig = figure["fig"]
        if handles and figure.get("show_legend", False):
            legend_position = figure.get("legend_position", DEFAULT_LEGEND_POSITION)
            legend_subplot_idx = figure.get("legend_subplot_idx")
            legend_ax = figure["axs"].get(legend_subplot_idx) if legend_subplot_idx is not None else None
            if legend_ax is None:
                legend_ax = list(figure["axs"].values())[-1]

            legend_ax.legend(
                handles, labels, loc=legend_position, fontsize=FONT_SIZE_LEGEND,
                frameon=True, fancybox=False, framealpha=1.0,
                facecolor='white', edgecolor='0.75', borderpad=0.5,
                handletextpad=0.6, handlelength=1.8
            )
...
def plot_PI_waterfall(model_name: str) -> None:
    ...
    for ax_idx, (param_name, runs) in enumerate(sorted(groups.items())):
        ...
        ax.scatter(xs, f_vals, color='black', s=12, zorder=3, alpha=0.6)
