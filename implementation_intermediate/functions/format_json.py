...
def format_float(value: float) -> str:
    # check if the float uses scientific notation
    if 'e' in str(value):
        whole_part, fractional_part = ('{:.20f}'.format(value)).split('.')
    else:
        # Split the value into whole part and fractional part
        whole_part, fractional_part = str(value).split('.')

    # Initialize variables to hold the start indices of the first non-zero digit in the fractional part
    start_fractional = 0
    ...
    # Format the float with the correct number of decimal places
    formatted_value = whole_part + '.' + fractional_part[:start_fractional+1]

    # return the string with the formatted float as a string
